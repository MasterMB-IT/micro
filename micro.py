...
# --- CSS DEFINITIVO PER PULSANTI IN LINEA ---
st.markdown("""
    <style>
    ...
    /* Header */
    .aosr-header {
        background: rgba(26, 31, 44, 0.95); padding: 15px; border-radius: 15px; 
        border: 2px solid #00c8ff; text-align: center; margin-bottom: 20px;
    }
    .aosr-title { font-family: 'Orbitron', sans-serif; color: #00c8ff; font-size: 1.8rem; }
    ...
    .print-container { background-color: #000; padding: 20px; border-radius: 15px; border: 2px solid #00c8ff; }
    
    /* CARD GIORNO */
    .summary-card {
        background: #111; border: 1px solid #333; padding: 12px; 
        border-radius: 10px; margin-bottom: 10px; min-height: 100px;
    }
    .day-label { color: #00c8ff; font-weight: bold; font-size: 0.8rem; text-align: center; border-bottom: 1px solid #222; margin-bottom: 8px; padding-bottom: 4px; }

    /* STILE NOMI E MICRO-BOTTONI IN LINEA */
    .name-row {
        display: flex; align-items: center; justify-content: space-between;
        margin: 4px 0; min-height: 28px;
    }
    .name-text { font-size: 0.8rem; font-weight: bold; text-transform: uppercase; flex-grow: 1; }

    /* Rimuoviamo il fluo dai micro-bottoni */
    .stButton > button { border-radius: 4px !important; }
    .micro-btn div[data-testid="stHorizontalBlock"] { gap: 2px !important; }
    ...
    .micro-btn button {
        ...
        border: 1px solid #444 !important; color: #666 !important;
        box-shadow: none !important; /* Rimuove alone fluo */
    }
    .micro-btn button:hover { border-color: #00c8ff !important; color: #00c8ff !important; background: #222 !important; }
...
with col_b1:
    ...
    if st.button("🚀 GENERA"):
        ...
        for g in range(1, num_gg + 1):
            ...
            else: c, p = others[p_idx % len(others)], others[(p_idx+1) % len(others)]; p_idx += 2
...
if 'master_cal' in st.session_state:
    st.markdown(f"### 🖼️ CALENDARIO {st.session_state['sel_mese'].upper()} {st.session_state['sel_anno']}")
    ...
    for i, r in enumerate(st.session_state['master_cal']):
        with cols[i % 6]:
            ...
            st.markdown(f'<div class="summary-card"><div class="day-label">GG {r["Giorno"]}</div>', unsafe_allow_html=True)
            
            # Riga Capo + Bottone Inverti
            c1, c2 = st.columns([4, 1])
            c1.markdown(f'<div class="name-text" style="color:{c_col};">{r["Capo"]}</div>', unsafe_allow_html=True)
            with c2:
                st.markdown('<div class="micro-btn">', unsafe_allow_html=True)
                if st.button("🔄", key=f"inv_{i}"):
                    st.session_state['master_cal'][i]['Capo'], st.session_state['master_cal'][i]['Pass'] = r['Pass'], r['Capo']
                    st.rerun()
                st.markdown('</div>', unsafe_allow_html=True)

            st.markdown('<div style="color:#444; font-size:0.5rem; text-align:center;">&</div>', unsafe_allow_html=True)

            # Riga Pass + Bottone Edit
            p1, p2 = st.columns([4, 1])
            p1.markdown(f'<div class="name-text" style="color:{p_col};">{r["Pass"]}</div>', unsafe_allow_html=True)
            with p2:
                st.markdown('<div class="micro-btn">', unsafe_allow_html=True)
            ...
            # Form modifica (appare solo se cliccato)
            if st.session_state.get(f"edit_{i}", False):
                nc = st.selectbox("Capo", all_names, index=all_names.index(r['Capo']), key=f"selc_{i}")
                np = st.selectbox("Pass", all_names, index=all_names.index(r['Pass']), key=f"selp_{i}")
