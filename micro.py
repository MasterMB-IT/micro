...
st.markdown("""
    ...
    /* VISIONE D'INSIEME FIXATA */
    .summary-section {
        background: #000000;
        padding: 20px; border-radius: 15px; border: 2px solid #00c8ff; margin-top: 30px;
    }
    .summary-grid {
        display: flex;
        flex-wrap: wrap;
    ...
    .summary-day-num { font-size: 0.8rem; color: #00c8ff; font-weight: bold; margin-bottom: 5px; border-bottom: 1px solid #222;}
    .summary-name { font-size: 0.75rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; font-weight: 700; display: block; }
    </style>
...
with st.expander("🛠️ CONFIGURAZIONE", expanded=True):
    ...
    with col_b:
        ...
        sel_meritevoli = st.multiselect("Seleziona Partecipanti (Vuoto per TUTTI)", meritevoli_opzioni)
...
if st.button("🚀 GENERA CALENDARIO AOSR", use_container_width=True):
    ...
    for g in range(1, num_gg + 1):
        if g <= 11:
            ...
            c = pool[pool_idx % len(pool)]; pool_idx += 1
            p = pool[pool_idx % len(pool)]; pool_idx += 1
        res_cal.append({"Giorno": g, "Capotreno": c, "Passeggero": p})
    st.session_state['master_cal'] = res_cal

# --- LOGICA VISUALIZZAZIONE ---
if 'master_cal' in st.session_state:
    st.markdown("### 📅 Gestione Marce")
    
    for i, r in enumerate(st.session_state['master_cal']):
        cols = st.columns([0.5, 2, 2, 0.5, 0.5])
        cols[0].write(f"**G{r['Giorno']}**")
        ...
        for idx, role in enumerate(['Capotreno', 'Passeggero']):
            ...
            cols[idx+1].markdown(f'<div class="p-box {style}">{nome}</div>', unsafe_allow_html=True)
            
        # Tasti Inverti e Modifica
        if cols[3].button("🔄", key=f"inv_{i}"):
            ...
            st.rerun()
        if cols[4].button("✏️", key=f"ed_btn_{i}"):
        ...
        if st.session_state.get(f"edit_mode_{i}", False):
            ...
            nc = e1.selectbox("Capo", all_names, index=all_names.index(r['Capotreno']), key=f"nc_{i}")
            np = e2.selectbox("Pass", all_names, index=all_names.index(r['Passeggero']), key=f"np_{i}")
            if e3.button("✅", key=f"sv_{i}"):
                st.session_state['master_cal'][i].update({"Capotreno": nc, "Passeggero": np})
