...
st.set_page_config(page_title="Train Manager Ultra Pro v5", layout="wide", page_icon="🚂")

# CSS Gaming Style (Invariato per coerenza estetica)
st.markdown("""
    <style>
...
# --- ANTE-GENERAZIONE: LEADER CON AUTO-ACCOPPIAMENTO DIVERSO ---
with st.expander("🛠️ CONFIGURAZIONE R5/R4 (Giorni 1-11)", expanded=True):
    st.info("I passeggeri sono stati pre-impostati per essere tutti diversi e ruotati rispetto ai capotreni.")
    custom_leaders = []
    cols = st.columns(3)
    for i in range(11):
        with cols[i % 3]:
            st.markdown(f"**Giorno {i+1:02d}**")
            # Capotreno fisso in ordine di lista
            capo = st.selectbox(f"Capo G{i+1}", leaders_list, index=i, key=f"c{i}")
            
            # Logica per il passeggero pre-impostato: 
            # Prende il leader successivo (i+1), e se arriva alla fine ricomincia.
            # Questo garantisce che all'avvio siano tutti diversi tra loro e dai capi.
            default_pass_idx = (i + 1) % len(leaders_list)
            
            # Filtro per le opzioni (impedisce di scegliere lo stesso nome)
            pass_opt = [n for n in leaders_list if n != capo]
            
            # Calcoliamo l'indice di default all'interno della lista filtrata
            # (cerchiamo il nome che sta all'indice default_pass_idx nella lista originale)
            nome_target = leaders_list[default_pass_idx]
            try:
                idx_in_filtered = pass_opt.index(nome_target)
            except:
                idx_in_filtered = 0

            passeg = st.selectbox(f"Pass G{i+1}", pass_opt, index=idx_in_filtered, key=f"p{i}")
...
if st.button("🚀 GENERA CALENDARIO COMPLETO", use_container_width=True):
    ...
    # Pool meritevoli per unicità (nessuna ripetizione nel mese)
    pool_meritevoli = list(meritevoli_scelti)
    random.shuffle(pool_meritevoli)
    ...
    for giorno in range(1, num_giorni + 1):
        if giorno <= 11:
            ...
            capo = config['capotreno']
            passg = config['passeggero']
        else:
            # Assegnazione R3 senza ripetizioni (pop)
            if len(pool_meritevoli) >= 2:
                capo = pool_meritevoli.pop(0)
                passg = pool_meritevoli.pop(0)
        ...
        giorni_info.append({"Giorno": giorno, "Capotreno": capo, "Passeggero": passg})

    st.session_state['data_cal'] = giorni_info

# --- RENDERING E EDITING POST-GENERAZIONE ---
if 'data_cal' in st.session_state:
    st.subheader("📝 Modifica Rapida (Tutti i giorni modificabili)")
    ...
    edited_df = st.data_editor(
        ...
        disabled=["Giorno"], hide_index=True, use_container_width=True, key="editor_v5"
    )
    ...
    for idx, sett in enumerate(settimane):
        ...
        for d in sett:
            c_badge = "leader-badge" if d['Capotreno'] in leaders_list else "merit-badge"
            p_badge = "leader-badge" if d['Passeggero'] in leaders_list else "merit-badge"
            st.markdown(f"""
                <div class="day-card">
                    ...
                    <div style="flex-grow:1; display:flex; gap:40px;">
                        <div><span style="color:#808080; font-size:0.75rem;">CAPOTRENO</span><br><span class="{c_badge}">{d['Capotreno']}</span></div>
                        <div><span style="color:#808080; font-size:0.75rem;">PASSEGGERO</span><br><span class="{p_badge}">{d['Passeggero']}</span></div>
                    </div>
                </div>
            """, unsafe_allow_html=True)
